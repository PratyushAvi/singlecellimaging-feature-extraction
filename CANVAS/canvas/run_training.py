...
def get_args_parser():
    ...
    # NEW ARGUMENTS
    parser.add_argument('--tile_size', type=int, default=44, help="image size")
    parser.add_argument('--embedding_dim', type=int, help="output dimension", default=1024)
...
def main():
    ...
    # Run training script
    subprocess.run(f'torchrun --standalone --nnodes=1 --nproc_per_node={config.ngpus} {current_path}/model/main_pretrain.py --epoch {config.epoch} --batch_size {config.batch_size} --tile_size {tile_size} --output_dir {ckpt_path} --log_dir {log_path} --data_path {data_path} --quantification_path "/scratch/pa2439/Projects/single_cell_imaging/feaxt/data/tonsil/data/raw_data/crop-quantification-crc01-097-096.csv" --channel_name_start_index 4 --channel_name_end_index 47', shell=True)
